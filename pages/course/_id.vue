...
export default {
  ...
  methods: {
    shi(vi) {
      if(this.isbuy || Number(this.courseWebVo.price) === 0){
        let v=vi.videoSourceId.slice(2)
      // console.log(v);
      this.$router.push({
        name:'player-vid',
        params:{vid:v}
    ...
    initCourseInfo() {
      ...
      // // console.log(axios);
      // if (userStr) {
      //   var use = JSON.parse(userStr)
      //   axios.get('http://localhost:3000/eduorder/paylog/isBuyCourse/'+this.courseId+'/'+use.id).then(response => {
      //     console.log(response.data.data.isBuy);
      //     this.isbuy = response.data.data.isBuy
      //   })
      //   // ordersApi.isBuyCourse(this.courseId, use.id).then(response => {
      //   //   console.log(response.data.data.isBuy);
      //   // this.isbuy = response.data.data.isBuy
      //   // })
      // }
